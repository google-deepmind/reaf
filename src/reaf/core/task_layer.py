...
"""Task layer for the Robotics Environment Authoring Framework."""
...
class TaskLayer:
  ...
  def __init__(
    ...
    Args:
      commands_processors: `CommandsProcessor`s that modify the commands before
        being sent down to the DeviceLayer. They are called sequentially,
        starting from the commands supplied by the policy and ending with the
        commands that will be sent to the DeviceLayer.
      features_producers: `FeaturesProducer`s that generate new features.
        Measurements collected by the DeviceLayer and features produced by these
  ...
  def perform_reset(self) -> None:
    """Reset the internal state of the task layer."""
